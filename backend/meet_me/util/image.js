...
exports.writeFile = (file, mode, id, ext) => {
    ...
    // 저장될 파일 이름 생성
    const fileName = `${mode}/${id}/${date}-${v4()}${ext}`;

    const dir = path.join(__dirname, `./uploads/${mode}/${id}`);
    ...
    const data = file.data.slice(file.data.indexOf(';base64') + 8);
    
    // 파일 저장 부분. 에러 발생 시 그대로 리턴하도록 만듦
    fs.writeFile(fullPath, data, 'base64', async(err)=>{
        if (err){
            console.error(err);
            throw new Error("파일 저장 에러");
        }
    });
    return fullPath;
}
